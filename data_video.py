...
class SFTDataset(Dataset):
    def __init__(self, data_dir, video_size, fps, max_num_frames, max_subjects=3, skip_frms_num=3):
        """
        skip_frms_num: ignore the first and the last xx frames, avoiding transitions.
        """
        ...
        self.video_paths = []
        self.captions = []
        self.references = []  # list of list of reference image paths

        video_dir = os.path.join(data_dir, "videos")
        for filename in sorted(os.listdir(video_dir)):
            if filename.endswith(".mp4"):
                video_path = os.path.join(video_dir, filename)
                base_id = os.path.splitext(filename)[0]  # '1', '2', ...
                
                self.video_paths.append(video_path)

                caption_path = os.path.join(data_dir, "labels", base_id + ".txt")
                ...
                # reference 
                ref_dir = os.path.join(data_dir, "references", base_id)
                refs = []
                if os.path.isdir(ref_dir):
                    for i in range(1, 4):  # ref1, ref2, ref3 
                        img_name = f"ref{i}.png"
                        img_path = os.path.join(ref_dir, img_name)
                        if os.path.exists(img_path):
                            refs.append(img_path)
                self.references.append(refs)
    def _load_reference_images(self, ref_paths):
        h, w = self.video_size  # target height, width
        tensors = []
        for p in ref_paths:
            ...
            tensor = torch.from_numpy(np.array(new_img)).permute(2,0,1).contiguous().float()  # [C,H,W]
    ...
    def __getitem__(self, index):
        ...
        ## reference 
        ref_tensors = self._load_reference_images(self.references[index])
        item = {
            "mp4": tensor_frms,
            "txt": self.captions[index],
            "num_frames": num_frames,
            "fps": self.fps,
            'subject_images': ref_tensors,
        }
        return item
